...
def _iso_to_datetime(iso: Optional[str]) -> Optional[datetime]:
...
def _json_to_text(data: Optional[Dict[str, Any]]) -> Optional[str]:
    """Convert dict to JSON string."""
    return json.dumps(data) if data is not None else None


def _text_to_json(text: Optional[str]) -> Optional[Dict[str, Any]]:
    ...
    def list_all(self, filters: Optional[dict] = None) -> List[Run]:
        ...
        if filters:
            if "status" in filters:
                query += " AND status = ?"
                params.append(filters["status"].value if isinstance(filters["status"], RunStatus) else filters["status"])
